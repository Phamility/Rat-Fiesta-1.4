...
namespace TenShadows.NPCS
{
    public class GlobalNPCS : GlobalNPC
    {
        ...
        public override void ModifyNPCLoot(NPC npc, NPCLoot npcLoot)
        {
            ...
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<CursedEnergy>(), 50, 1, 3));

            }
            if (npc.type == NPCID.Zombie || npc.type == NPCID.DemonEye)
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<CursedEnergy>(), 25, 3, 6));

            }
            if (npc.type == NPCID.BloodZombie || npc.type == NPCID.Drippler)
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<CursedEnergy>(), 15, 6, 9));

            }
            if (npc.type == NPCID.FaceMonster || npc.type == NPCID.EaterofSouls || npc.type == NPCID.Crimera)
            {
                npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<CursedEnergy>(), 20, 6, 9));
